...
const Post = () => {
  ...
  const usernamePost = useSelector(
    (state) => state.post?.obj?.author?.username,
  );

  const usernameUser = useSelector((state) => state.user?.user?.username);
  const { currentPage, limit } = useSelector((state) => state.articles);

  const isAuthName = usernamePost === usernameUser ? isAuth : "";

  const { loading, obj, error } = useSelector((state) => state.post);
  ...
  if (!obj) return <p>Пост не найден</p>;

  const isModal = () => {
    setShowModal(!showModal);
  };

  const confirmationDel = async (delArticleConfirm) => {
    if (delArticleConfirm) {
    ...
    <div className={`${styles["post"]}`}>
      <div className={`${styles["posts__item-header"]}`}>
        <div className={`${styles["posts__item-header-title-block"]}`}>
          <h4 className={`${styles["post__item-header-block-title"]}`}>
            {obj.title}
          </h4>
          <div className={`${styles["posts__item-header-rating"]}`}>
            <button
              className={`${styles["btn__like"]}`}
              onClick={() => toggleLikePost(obj.slug, obj.favorited)}
              type="button"
            >
              <svg
                className={
            ...
            <span className={`${styles["posts__item-header-rating-count"]}`}>
              {obj.favoritesCount}
            </span>
          </div>
        </div>
        <div className={`${styles["posts__item-header-user"]}`}>
          <div className={`${styles["posts-header-user-block"]}`}>
            ...
            <span className={`${styles["posts__item-header-user-date"]}`}>
              {format(new Date(obj.createdAt), "MMMM dd,yyyy")}
            </span>
          </div>
          <a>
            <img
              ...
              alt={`images${obj.username}`}
            />
          </a>
        </div>
      </div>

      <div className={`${styles["post__item-descr"]}`}>
        <ul className={`${styles["posts__item-descr-tags"]}`}>
          {obj.tagList.map((tag, index) => {
            return (
              <li
                key={index}
                className={
        ...
        <div className={`${styles["posts__item-descr-block"]}`}>
          <p className={`${styles["posts__item-descr-text"]}`}>
            {obj.description}
          </p>
          {isAuthName && (
            <div className={`${styles["posts__item-descr-block-btns"]}`}>
              <DeleteBtn showModalDel={isModal} />
              {showModal && <ModalDelete confirmDel={confirmationDel} />}
              <EditBtn obj={obj} slug={obj.slug} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="post__main">
        <div className="post__main-markdown">
          <ReactMarkdown>{obj.body}</ReactMarkdown>
        </div>
      </div>
    </div>
  );
